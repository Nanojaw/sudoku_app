...
struct Generator {
    ...
    void Generate(std::vector<std::string>& puzzles) {
        char puzzle[729];
        ...
        for (uint64_t i = 0; i < options_.max_puzzles; i++) {
            ...
            if (!options_.display_all) {
                std::string puzzleStr(puzzle);

                puzzles.push_back(puzzleStr);
            }
...
const char* generateNPuzzles()
{
    ...
    generator.Generate(sudokus);

    auto sudoku = new std::string(sudokus[0]);
